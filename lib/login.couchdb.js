...
module.exports = (function (app, couchdb_url, postmark, options) { 
  ...
  var actions = {
    ...
    authentication: function (req, res) {
      ...
      if (req.body.email && req.body.password) {
        http.get(new CouchDBRequest(req.body.email), function (response) {
            ...
            response.on("end", function () {
              ...
              if (user && user.email == req.body.email) {
                if (bcrypt.compare_sync(req.body.password, user.encrypted_password)) {
                  ...
                  var update = http.request(new CouchDBRequest(req.body.email, 'POST'), function (post_response) {
    ...
    reset_password: function (req, res) {
      ...
      if (req.params.email && req.params.psk) {
        http.get(new CouchDBRequest(req.params.email), function (response) {
          if (response.statusCode == 200) {
            var data = "";
            response.on("data", function (d) { data += d; });
            response.on("end", function () {
  ...
  var fubt = (function (pt, cb) {
    var parts = pt.split(":");
    http.get(new CouchDBRequest(parts[0]), function (response) {
      data = "";
      response.on("data", function (d) { data += d; });
      response.on("end", function () {
        var user = JSON.parse(data);
        if (user.persistence_token == parts[1] && user.current_login_at >= (+helpers.session_timeout())) {
          cb(user);
        } else {
          cb(null);
        }
    ...
    bootstrap: function (email, password, extra_fields) {
     http.get(new CouchDBRequest(email), function (response) {
        if (response.statusCode == 404) {
          ...
          var userCreation = http.request(new CouchDBRequest(email, 'POST'), function () {
            console.log("Bootstrapped user authentication system");
          });
          userCreation.write(JSON.stringify({
            "_id": email,

        ...
        redis.hgetall(req.body.email, function (err, user) {
          if (!err && user.email == req.body.email) {
            if (bcrypt.compare_sync(req.body.password, user.encrypted_password)) {
              ...
              redis.hmset(req.body.email, "helpers.persistence_token", pt, "current_login_at", ""+now, "last_login_at", user.current_login_at, "last_login_ip", user.current_login_ip, "current_login_ip", ipAddr, function (err) {
      ...
      if (req.body.psk && req.body.email) {
        hgetall(req.body.email, function (err, user) {
          if (user.last_request_at < (+helpers.yesterday()) || req.params.psk != user.perishable_token) {
            ...
            var pt = helpers.persistence_token()
            redis.hmset(req.body.email, "encrypted_password", hash,"helpers.persistence_token", pt, "current_login_at", now, "last_login_at", user.current_login_at, "last_login_ip", user.current_login_ip, "current_login_ip", ipAddr, function (err) {
              req.session.pt = req.body.email+":"+pt;

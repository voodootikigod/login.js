  ...
  var actions = {
    ...
    authentication: function (req, res) {
      if (req.body.email && req.body.password) {
        postgresql.query("select * from users where email = $1 and disabled_at is null and deleted_at is null", [req.body.email], function (err, result) {
          if (result.rows.length == 1) {
            ...
            if (bcrypt.compare_sync(req.body.password, user.crypted_password)) {
              ...
              postgresql.query("update users set persistence_token = $1, current_login_at = $2, last_login_at = $3, last_login_ip = $4, current_login_ip = $5 where id = $6;", [pt, now, user.current_login_at, user.current_login_ip, ipAddr, user.id ], function (err, update_res) {
                if (!err) {
                  req.session.pt = pt;
                  res.redirect("/");
                } else { 
    ...
    send_password: function (req, res) {
      if (req.body.email) {
        postgresql.query("select * from users where email = $1", [req.body.email], function (err, result) {
          if (result.rows.length == 1) {
            var user = result.rows[0];
